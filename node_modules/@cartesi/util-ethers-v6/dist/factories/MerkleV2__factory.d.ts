import { ContractFactory, ContractTransactionResponse } from "ethers";
import type { Signer, ContractDeployTransaction, ContractRunner } from "ethers";
import type { NonPayableOverrides } from "../common";
import type { MerkleV2, MerkleV2Interface } from "../MerkleV2";
type MerkleV2ConstructorParams = [linkLibraryAddresses: MerkleV2LibraryAddresses, signer?: Signer] | ConstructorParameters<typeof ContractFactory>;
export declare class MerkleV2__factory extends ContractFactory {
    constructor(...args: MerkleV2ConstructorParams);
    static linkBytecode(linkLibraryAddresses: MerkleV2LibraryAddresses): string;
    getDeployTransaction(overrides?: NonPayableOverrides & {
        from?: string;
    }): Promise<ContractDeployTransaction>;
    deploy(overrides?: NonPayableOverrides & {
        from?: string;
    }): Promise<MerkleV2 & {
        deploymentTransaction(): ContractTransactionResponse;
    }>;
    connect(runner: ContractRunner | null): MerkleV2__factory;
    static readonly bytecode = "__$e74161300f6257ab2934e4b08c2b2efd56$____$e74161300f6257ab2934e4b08c2b2efd56$____$e74161300f6257ab2934e4b08c2b2efd56$__";
    static readonly abi: readonly [{
        readonly inputs: readonly [{
            readonly internalType: "bytes32[]";
            readonly name: "hashes";
            readonly type: "bytes32[]";
        }];
        readonly name: "calculateRootFromPowerOfTwo";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "uint256";
            readonly name: "_index";
            readonly type: "uint256";
        }];
        readonly name: "getEmptyTreeHashAtIndex";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "bytes";
            readonly name: "_data";
            readonly type: "bytes";
        }, {
            readonly internalType: "uint256";
            readonly name: "_wordIndex";
            readonly type: "uint256";
        }];
        readonly name: "getHashOfWordAtIndex";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "bytes";
            readonly name: "_data";
            readonly type: "bytes";
        }, {
            readonly internalType: "uint256";
            readonly name: "_log2Size";
            readonly type: "uint256";
        }];
        readonly name: "getMerkleRootFromBytes";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "uint256";
            readonly name: "_position";
            readonly type: "uint256";
        }, {
            readonly internalType: "uint256";
            readonly name: "_logSizeOfReplacement";
            readonly type: "uint256";
        }, {
            readonly internalType: "uint256";
            readonly name: "_logSizeOfFullDrive";
            readonly type: "uint256";
        }, {
            readonly internalType: "bytes32";
            readonly name: "_replacement";
            readonly type: "bytes32";
        }, {
            readonly internalType: "bytes32[]";
            readonly name: "siblings";
            readonly type: "bytes32[]";
        }];
        readonly name: "getRootAfterReplacementInDrive";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }];
    static createInterface(): MerkleV2Interface;
    static connect(address: string, runner?: ContractRunner | null): MerkleV2;
}
export interface MerkleV2LibraryAddresses {
    ["contracts/CartesiMathV2.sol:CartesiMathV2"]: string;
}
export {};
//# sourceMappingURL=MerkleV2__factory.d.ts.map