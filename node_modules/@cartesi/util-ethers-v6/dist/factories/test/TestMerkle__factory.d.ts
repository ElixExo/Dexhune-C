import { ContractFactory, ContractTransactionResponse } from "ethers";
import type { Signer, ContractDeployTransaction, ContractRunner } from "ethers";
import type { NonPayableOverrides } from "../../common";
import type { TestMerkle, TestMerkleInterface } from "../../test/TestMerkle";
type TestMerkleConstructorParams = [linkLibraryAddresses: TestMerkleLibraryAddresses, signer?: Signer] | ConstructorParameters<typeof ContractFactory>;
export declare class TestMerkle__factory extends ContractFactory {
    constructor(...args: TestMerkleConstructorParams);
    static linkBytecode(linkLibraryAddresses: TestMerkleLibraryAddresses): string;
    getDeployTransaction(overrides?: NonPayableOverrides & {
        from?: string;
    }): Promise<ContractDeployTransaction>;
    deploy(overrides?: NonPayableOverrides & {
        from?: string;
    }): Promise<TestMerkle & {
        deploymentTransaction(): ContractTransactionResponse;
    }>;
    connect(runner: ContractRunner | null): TestMerkle__factory;
    static readonly bytecode = "__$a7cee7f7ec908a01620b3c597839795faf$__906334bd712d9061010b908790879087906004016104b2565b602060405180830381865af4158015610128573d6000803e3d6000fd5b505050506040513d601f19601f8201168201806040525081019061014c91906104eb565b949350505050565b60405163451a105560e01b81526004810182905260009073__$a7cee7f7ec908a01620b3c597839795faf$__9063451a1055906024015b602060405180830381865af41580156101a8573d6000803e3d6000fd5b505050506040513d601f19601f820116820180604052508101906101cc91906104eb565b92915050565b6040516379de460160e01b815260009073__$a7cee7f7ec908a01620b3c597839795faf$__906379de460190610214908990899089908990899060040161053f565b602060405180830381865af4158015610231573d6000803e3d6000fd5b505050506040513d601f19601f8201168201806040525081019061025591906104eb565b9695505050505050565b6040516310561d5760e31b815260009073__$a7cee7f7ec908a01620b3c597839795faf$__906382b0eab89061018b90859060040161056a565b60405163c84583a160e01b815260009073__$a7cee7f7ec908a01620b3c597839795faf$__";
    static readonly abi: readonly [{
        readonly inputs: readonly [{
            readonly internalType: "bytes32[]";
            readonly name: "hashes";
            readonly type: "bytes32[]";
        }];
        readonly name: "calculateRootFromPowerOfTwo";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "uint256";
            readonly name: "_index";
            readonly type: "uint256";
        }];
        readonly name: "getEmptyTreeHashAtIndex";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "bytes";
            readonly name: "_data";
            readonly type: "bytes";
        }, {
            readonly internalType: "uint256";
            readonly name: "_wordIndex";
            readonly type: "uint256";
        }];
        readonly name: "getHashOfWordAtIndex";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "bytes";
            readonly name: "_data";
            readonly type: "bytes";
        }, {
            readonly internalType: "uint256";
            readonly name: "_log2Size";
            readonly type: "uint256";
        }];
        readonly name: "getMerkleRootFromBytes";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }, {
        readonly inputs: readonly [{
            readonly internalType: "uint256";
            readonly name: "_position";
            readonly type: "uint256";
        }, {
            readonly internalType: "uint256";
            readonly name: "_logSizeOfReplacement";
            readonly type: "uint256";
        }, {
            readonly internalType: "uint256";
            readonly name: "_logSizeOfFullDrive";
            readonly type: "uint256";
        }, {
            readonly internalType: "bytes32";
            readonly name: "_replacement";
            readonly type: "bytes32";
        }, {
            readonly internalType: "bytes32[]";
            readonly name: "siblings";
            readonly type: "bytes32[]";
        }];
        readonly name: "getRootAfterReplacementInDrive";
        readonly outputs: readonly [{
            readonly internalType: "bytes32";
            readonly name: "";
            readonly type: "bytes32";
        }];
        readonly stateMutability: "pure";
        readonly type: "function";
    }];
    static createInterface(): TestMerkleInterface;
    static connect(address: string, runner?: ContractRunner | null): TestMerkle;
}
export interface TestMerkleLibraryAddresses {
    ["contracts/MerkleV2.sol:MerkleV2"]: string;
}
export {};
//# sourceMappingURL=TestMerkle__factory.d.ts.map